...
def test_release_checklist_api_available(tmp_path):
    ...
    assert body["commit_message"] == "Add batch export and production handoff workflow"
...
def test_setup_guide_api(tmp_path):
    ...
    assert response.status_code == 200
    assert response.json()["setup"]["commit_message"] == "Add batch export and production handoff workflow"
